...
10. `state/00-sql-execution-lifecycle.puml`：DefaultRelationalTemplate 完整生命周期。
11. `state/01-connection-handle-lifecycle.puml`：OWNED/BORROWED 的真正意义。
12. `sequence/05-spring-shared-transaction.puml`：业务 MyBatis 与 RelationalTemplate 如何共享同一 Connection。
13. `sequence/06-idempotency-txabc-migration.puml`：当前幂等 JdbcExecutionManager 的 Tx-A/Tx-B/Tx-C 如何迁移。
14. `sequence/01-idempotency-storage-flow.puml`：幂等语义 -> Storage -> SQL -> Relational Access。
