...
13. `design/02-real-mysql-integration-test.md`：真实 MySQL 多库同事务验证方式。
14. `state/00-sql-execution-lifecycle.puml`：DefaultRelationalTemplate 完整生命周期。
15. `state/01-connection-handle-lifecycle.puml`：OWNED/BORROWED 的真正意义。
16. `sequence/05-spring-shared-transaction.puml`：业务 MyBatis 与 RelationalTemplate 如何共享同一 Connection。
17. `sequence/06-idempotency-txabc-migration.puml`：当前幂等 JdbcExecutionManager 的 Tx-A/Tx-B/Tx-C 如何迁移。
18. `sequence/01-idempotency-storage-flow.puml`：幂等语义 -> Storage -> SQL -> Relational Access。
...
```text
仓库只提交环境变量名，不提交真实密码
未配置 IRON_TEST_MYSQL_* 时自动跳过
配置后验证真实 MySQL 上的同 shard 提交、同 shard 回滚、错 shard 风险
```

真实数据库 URL、用户名、密码不进入组件源码仓库。组件测试默认使用 H2 多内存库验证路由语义；配置环境变量后，可以额外使用真实 MySQL 验证多库同事务行为。生产环境由业务工程通过 Spring Bean / 配置中心 / Secret 注入 DataSource。
